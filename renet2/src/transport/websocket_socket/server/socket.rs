...
use crate::transport::{client_idx_from_addr, client_idx_to_addr, TransportSocket, HTTP_CONNECT_REQ};
...
/// Implementation of [`TransportSocket`] for WebSocket servers.
///
...
impl WebSocketServer {
    ...
    pub fn new(config: WebSocketServerConfig, handle: tokio::runtime::Handle) -> Result<Self, Error> {
        ...
        let socket = handle.block_on(async { tokio::net::TcpListener::bind(addr).await })?;
...
impl TransportSocket for WebSocketServer {
    ...
    fn try_recv(&mut self, buffer: &mut [u8]) -> std::io::Result<(usize, SocketAddr)> {
        ...
        for (client_idx, client_data) in self
            .clients
            .range((core::ops::Bound::Included(&start_index), core::ops::Bound::Excluded(&end_index)))
        {
    ...
    fn send(&mut self, addr: SocketAddr, packet: &[u8]) -> Result<(), crate::transport::NetcodeTransportError> {
        ...
        match client_data.outgoing_sender.try_send(data) {
            ...
            Err(mpsc::error::TrySendError::Full(_)) => {
                log::trace!("dropping packet for client {client_idx}; writer thread is backed up, client maybe unresponsive");
            }
            Ok(()) => (),
        }
